...
const drawData = () =>{
    ...
    allUsers.forEach((user, i)=>{
        ...
        const status = createMyOwnElement('td', tr, user.status)
        ...
        editBtn.addEventListener('click', (e) => {
            ...
            const act = readFromStorage('users');
            let state = act[edit].status
            if (state === 'deactive') {
                status.textContent = 'active'
                act[edit].status = "active"
            } else if (state === 'active') {
                status.textContent = 'deactive'
                act[edit].status = "deactive"
            } 
            writeToStorage(act, "users")
        })
